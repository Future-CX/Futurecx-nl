...
from ecommerce_trends_radar import EcommerceTrendsRadarCrew
...
def ecommerce_trends_radar():
    """
    Run Ecommerce Trends Radar flow.
    """
    inputs = {
        'topic': 'Ecommerce Trends',
        'current_year': str(datetime.now().year)
    }

    try:
        EcommerceTrendsRadarCrew().crew().kickoff(inputs=inputs)
    except Exception as e:
        raise Exception(f"An error occurred while running the crew: {e}")